...
if [ ! -d "$SAVE_DIR" ]; then
	mkdir -p "$SAVE_DIR" || { echo -e "\e[32m 디렉터리 생성 실패: $SAVE_DIR\e[0m"; exit 1; }
fi
...
if [ $? -eq 0 ]; then
	...
	chmod -R 777 "$SAVE_DIR" || { echo -e "\e[32m권한 변경 실패: $SAVE_DIR\e[0m"; exit 1; }
else
	echo -e "\e[32m저장 중 오류가 발생했습니다. 로그를 확인하세요: $HOME/backup/error.log\e[0m"
	exit 1
fi
